...
import RegisterModal from '../components/RegisterModal'; // استيراد المودال اللي عملناه
...
const Navbar = () => {
  ...
  const toggleLanguage = () => {
    ...
    setIsMobileMenuOpen(false); // نقفل المنيو لو غيرنا اللغة في الموبايل
  };
  ...
  return (
    <>
      ...
      >
          ...
          <div className="flex items-center gap-4 md:gap-8">
            ...
            <div className="hidden xl:flex items-center gap-8 text-white uppercase font-bold tracking-[0.15em] text-[10px]">
              ...
              <div className={`flex items-center gap-8 border-white/20 ${isRtl ? 'border-r pr-8' : 'border-l pl-8'}`}>
                ...
                <a href="https://wa.me/971XXXXXXXX" className="hover:opacity-60 transition-opacity flex items-center gap-2">
                  <MessageCircle size={14} /> {t.whatsapp}
                </a>
              </div>
            </div>

            {/* زر Register - الآن يفتح المودال */}
            <button 
        ...
        {/* --- Mobile Menu --- */}
        <div className={`lg:hidden absolute top-full left-0 w-full bg-black/95 transition-all duration-500 overflow-hidden ${isMobileMenuOpen ? 'max-h-screen border-t border-white/10 py-10 shadow-2xl' : 'max-h-0'}`}>
          <div className="flex flex-col items-center gap-8 text-white uppercase text-[12px] tracking-[0.2em] font-bold">
            ...
            <button 
               ...
               className="text-amber-500 border border-amber-500/30 px-6 py-2 rounded-sm"
            >
               {t.register}
            </button>

            <div className="flex gap-10 pt-6 border-t border-white/10 w-full justify-center items-center">
              <a href="tel:+971XXXXXXXX"><Phone size={20} /></a>
              <a href="https://wa.me/971XXXXXXXX"><MessageCircle size={20} /></a>
      ...
      {/* استدعاء المودال في نهاية المكون */}
      <RegisterModal 
        isOpen={isModalOpen} 
